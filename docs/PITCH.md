...
Indian farmers lose crops to diseases they cannot diagnose in time, and most agri apps never get used because they require downloads and high digital literacy. **Smart Crop Bot** puts an AI advisor inside Telegram: send a leaf photo, Hindi voice note, or text, and get practical advice in seconds — diagnosis, weather, and mandi context — with zero install. Built on a free-tier stack (Telegram Bot API, Gemini, Groq Whisper, edge-tts) so a realistic prototype ships in a hackathon weekend.

## Judge bullets

- **Problem:** Expert advice is scarce; messaging apps are already universal  
- **Solution:** Reactive Telegram bot (photo + voice + text)  
...
Telegram → FastAPI → Gemini/Groq (+ Whisper/TTS) → SQLite/Streamlit admin
